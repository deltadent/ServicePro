-- Safe Company Settings Migration
-- Only creates tables and functions if they don't already exist

-- Create company settings table (safe)
...
-- Create system config table (safe)
...
-- Create company templates table (safe)
...
-- Insert default company settings (safe)
...
-- Insert default system configs (safe)
...
) AS v(config_key, config_value, description, is_public)
WHERE NOT EXISTS (
    SELECT 1 FROM public.system_config WHERE config_key = v.config_key
);

-- Create functions (safe - will replace if exists)
